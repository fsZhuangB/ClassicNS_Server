...
def index():
    ...
    if form.validate_on_submit():
        itera = request.form['query']
        log_query = request.form['query']
        result = Particles.query.filter_by(log_likelihood=log_query).one()
        #result = Particles.query.filter_by(id=log_query).one()
        
        if result is None:
            print("result is none!")
        else:
            #jsonDumps = json.dumps({"iterations":result.id, "params":result.params, "log_likelihood":result.log_likelihood, "tiebreaker":result.tiebreaker})
            d = {"iterations":result.id, "params":result.params, "log_likelihood":result.log_likelihood, "tiebreaker":result.tiebreaker}
            #session['query'] = jsonDumps
            return d
